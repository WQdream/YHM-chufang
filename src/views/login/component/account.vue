...
const state = reactive({
	...
	ruleForm: {
		...
		password: localStorage.getItem('loginInfo')?JSON.parse(localStorage.getItem('loginInfo')).password:'',
	},
	loading: {
		signIn: false,
	},
...
const onSignIn = async () => {
	...
	try {
		let res = await useLoginApi().login(state.ruleForm)
		if(res.code !== 200){
		...
		localStorage.setItem('loginInfo', JSON.stringify(state.ruleForm));
